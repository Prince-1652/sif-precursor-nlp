...
- **Fields:** `id`, `original_text`, `normalized_text`, `report_type`, `processing_status`, `sif_potential`, `risk_band`, `ai_summary`, `ai_solution`.
- **Purpose:** Stores the core observation data. The `report_type` field explicitly drives the NLP engines, dynamically scaling Risk Bands and applying Life-Saving Rule exclusions based on context before generating the final aggregated status. Additionally, stores AI-generated summaries and suggested preventative solutions.
